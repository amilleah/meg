...
trials = None
try:
    with open(join(DIR, 'config', 'adjorder.json'), 'r') as f:
        config = json.load(f)
...
condition_map = config['send_triggers']['condition_map']
...
def is_match_trial(trial):
    return trial['match_mismatch'] == 'match'
...
def map_condition_to_trigger(condition_code):
    return trigger_map[condition_map[condition_code]]

def verify_trial_condition(trial):
    """Verify that the trial condition code is valid (1-5)"""
    if 'condition' not in trial or not (1 <= trial['condition'] <= 5):
        raise ValueError(f"Invalid trial condition code: {trial.get('condition')}")

def present_sentence(trial, use_frame_rate=True, send_trigger=True, verbose=True):
    try:
        verify_trial_condition(trial)
        ...
        if send_trigger:
            current_trigger = map_condition_to_trigger(trial['condition'])
            win.callOnFlip(trialClock.reset)
            win.callOnFlip(sendTrigger, channel=current_trigger, duration=0.02)
            if verbose:
                print(f"Sending trigger: {current_trigger} for condition: {trial['condition']}")
