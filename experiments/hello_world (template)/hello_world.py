...
dlg_info = OrderedDict([
    ('Participant ID', ''),
    ('Experiment Type', ['full']), # no practice implemented
    ('Fullscreen', False),
    ('Diode on?', True),
    ('Send triggers?', True),
    ('Auto respond?', False)
...
trials = None
try:
    with open(join(DIR, 'config', 'hello_world.json'), 'r') as f:
        config = json.load(f)
...
def present_answer(trial, simulate_responses=False):
    try:
        display_text = trial['stim'] # change this to second stimulus/task
...
def run_block(block_trials):
    ...
    for _, trial in block_trials.iterrows():
        ...
        correct = int(answer == trial['correct_resp'])
        total_correct += int(correct)
        
        if verbose:
            print(f"Current Trial: {trial_num} / {trials.shape[0]}")
            print(f"Presented Stimulus: {trial['stim']}")
        ...
        if exp:
            ...
            # Dynamically add fields from the trial dataframe
